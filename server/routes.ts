    ...
    try {
      if (toolType === 'png-to-pdf' || toolType === 'excel-to-pdf') {
        if (toolType === 'png-to-pdf') {
          ...
          // Enhanced Excel to PDF conversion with proper table formatting
          try {
            const workbook = XLSX.readFile(inputFile);
            console.log(`Processing Excel file: ${inputFile}, sheets: ${workbook.SheetNames.join(', ')}`);
            ...
            // Add document header
            currentPage.drawText('Excel Spreadsheet Converted to PDF', {
              x: pageMargin,
              y: yPosition,
              size: 16,
              font: helveticaBold,
            ...
            for (const sheetName of workbook.SheetNames) {
              ...
              // Check if we need a new page for this sheet
              ...
              const maxCols = Math.min(Math.max(...jsonData.map(row => row.length)), 8);
              const columnWidth = Math.min(contentWidth / maxCols, 120);
              const cellHeight = 18;
              ...
              if (jsonData.length > 0) {
                ...
                for (let col = 0; col < maxCols; col++) {
                  const cellValue = String(headerRow[col] || '');
                  const displayText = cellValue.length > 15 ? cellValue.substring(0, 12) + '...' : cellValue;
                  ...
                  if (col < maxCols - 1) {
                    currentPage.drawLine({
                      start: { x: xPosition + columnWidth, y: yPosition - cellHeight + 5 },
                      end: { x: xPosition + columnWidth, y: yPosition + 5 },
                      color: rgb(0.5, 0.5, 0.5),
                      thickness: 1,
                    });
                  }
              ...
              // Draw data rows
              const maxRows = Math.min(jsonData.length - 1, 25); // Skip header, limit rows
              ...
              for (let rowIndex = 1; rowIndex <= maxRows; rowIndex++) {
                ...
                if (addNewPageIfNeeded(cellHeight + 10)) {
                ...
                const rowColor = rowIndex % 2 === 0 ? rgb(0.98, 0.98, 0.98) : rgb(1, 1, 1);
                currentPage.drawRectangle({
                  x: pageMargin,
                  y: yPosition - cellHeight + 5,
                  width: contentWidth,
                ...
                for (let col = 0; col < maxCols; col++) {
                  ...
                  let displayText = cellValue.length > 15 ? cellValue.substring(0, 12) + '...' : cellValue;
                  ...
                  if (/^\d{1,2}:\d{2}\s?(AM|PM)$/i.test(cellValue)) {
              ...
              if (jsonData.length > maxRows + 1) {
                yPosition -= 10;
                currentPage.drawText(`... and ${jsonData.length - maxRows - 1} more rows`, {
                  x: pageMargin,
                  y: yPosition,
                  size: 8,
                  font: helveticaFont,
              ...
              yPosition -= 20; // Space between sheets
            }
            ...
            console.log(`Excel to PDF conversion completed successfully with ${pages.length} pages`);
            ...
            page.drawText('Excel to PDF Conversion Error', {
              x: 50,
              y: 750,
              size: 20,
              font: helveticaFont,
...
{\\i Generated by PDFo - PDF Processing Tool}
    ...
    case 'pdf-to-ppt':
      ...
      pptx.author = 'PDFo PDF Processor';
      pptx.company = 'PDFo - Free PDF Tools';
      pptx.title = 'PDF to PowerPoint Conversion';
      pptx.subject = 'Converted PDF Content';
      ...
      titleSlide.addText('Converted from PDF Document', {
        x: 1, y: 3, w: 8, h: 1,
        fontSize: 24,
        color: '666666',
        align: 'center'
      });
      titleSlide.addText(`Total Pages: ${pageCount} | Processing Date: ${new Date().toLocaleDateString()}`, {
        x: 1, y: 4, w: 8, h: 0.5,
        fontSize: 16,
        color: '999999',
        align: 'center'
      ...
      const textSections = actualText.split('\n\n').filter(section => section.trim());
      const maxSectionsPerSlide = 3;
      
      for (let i = 0; i < textSections.length; i += maxSectionsPerSlide) {
        const slide = pptx.addSlide();
        const slideNumber = Math.floor(i / maxSectionsPerSlide) + 1;
        
        // Slide title
        slide.addText(`Content - Slide ${slideNumber}`, {
        ...
        // Content sections
        const sectionsForSlide = textSections.slice(i, i + maxSectionsPerSlide);
        let yPosition = 1.5;
        
        sectionsForSlide.forEach((section, index) => {
        ...
        // Add page footer
        slide.addText(`Slide ${slideNumber} | Generated by PDFo`, {
      ...
      const summaryContent = [
        ...
        `• Content Slides: ${Math.ceil(textSections.length / maxSectionsPerSlide)}`,
        `• Word Count: ${actualText.split(/\s+/).length}`,
        `• Character Count: ${actualText.length}`
      ];
      
      summarySlide.addText(summaryContent.join('\n'), {
        x: 1, y: 2.5, w: 8, h: 3,
        fontSize: 18,
        color: '444444',
        valign: 'top'
      ...
      await pptx.writeFile({ fileName: finalOutput });
      break;
    default:
      fs.writeFileSync(finalOutput, actualText);
  }
...
async function unlockPdf(inputFile: string, outputFile: string, unlockOptions?: any): Promise<string> {
  try {
    const pdfBytes = fs.readFileSync(inputFile);
    console.log(`Unlocking PDF: ${inputFile}, size: ${pdfBytes.length} bytes`);
    ...
    try {
      ...
      console.log('PDF loaded without password protection');
    } catch (initialError) {
      console.log('PDF may be password protected, trying to bypass...');
      
      try {
        // Try to ignore encryption
        pdf = await PDFDocument.load(pdfBytes, { ignoreEncryption: true });
        wasPasswordProtected = true;
        console.log('PDF loaded with encryption bypass');
      } catch (bypassError) {
        console.log('Failed to bypass encryption, trying with password...');
        
        // If password is provided in options, try it
        if (unlockOptions?.password) {
          try {
            pdf = await PDFDocument.load(pdfBytes, { 
              ignoreEncryption: true,
              password: unlockOptions.password 
            });
            wasPasswordProtected = true;
            console.log('PDF unlocked with provided password');
          } catch (passwordError) {
            console.error('Password unlock failed:', passwordError);
            throw new Error('Invalid password provided. Please check the password and try again.');
          }
        } else {
          // Create a new PDF with unlock information
          console.log('Creating new PDF with unlock information');
          pdf = await PDFDocument.create();
          
          const page = pdf.addPage([612, 792]); // Standard letter size
          const helveticaFont = await pdf.embedFont(StandardFonts.Helvetica);
          
          // Add unlock attempt information
          page.drawText('PDF Unlock Attempt', {
            x: 50,
            y: 750,
            size: 24,
            font: helveticaFont,
          ...
          page.drawText('The original PDF file was password protected.', {
            x: 50,
            y: 700,
            size: 14,
            font: helveticaFont,
          ...
          page.drawText('To unlock this PDF, you need to provide the correct password.', {
            x: 50,
            y: 680,
            size: 14,
            font: helveticaFont,
          ...
          const securityInfo = [
            '• This PDF has password protection enabled',
            '• The document cannot be opened without the correct password',
            '• To unlock, please provide the password in the unlock options',
            '• Contact the document owner if you need the password',
            '',
            `• File size: ${Math.round(pdfBytes.length / 1024)} KB`,
            `• Unlock attempt: ${new Date().toLocaleString()}`,
            '• Tool: PDFo PDF Unlock System'
          ];
          
          securityInfo.forEach((info, index) => {
            page.drawText(info, {
              x: 70,
              y: 600 - (index * 20),
              size: 12,
          ...
          // Add PDFo branding
          page.drawText('Generated by PDFo - Free PDF Tools', {
            x: 50,
            y: 50,
            size: 10,
            font: helveticaFont,
    ...
    // If we successfully loaded the PDF, process it
    const pages = pdf.getPages();
    ...
    // Add unlock indicators to each page
    pages.forEach((page, index) => {
      const { width, height } = page.getSize();
      
      // Add subtle unlock confirmation (less intrusive)
      ...
      // If it was password protected, add a more prominent indicator
      if (wasPasswordProtected) {
        page.drawText('PASSWORD REMOVED', {
          x: width - 120,
          y: height - 30,
          size: 7,
          font: helveticaFont,
    ...
    const unlockedBytes = await pdf.save();
    fs.writeFileSync(outputFile, unlockedBytes);
    console.log(`PDF unlocked successfully: ${outputFile}`);
    return outputFile;
    
  } catch (error) {
    console.error('Error unlocking PDF:', error);
    
    // Create an error information PDF
    try {
      ...
      page.drawText('PDF Unlock Failed', {
        x: 50,
        y: 750,
        size: 24,
        font: helveticaFont,
      ...
      page.drawText('The PDF file could not be unlocked.', {
        x: 50,
        y: 700,
        size: 14,
        font: helveticaFont,
      ...
      const errorDetails = [
        ...
        '• The password protection may be too strong',
        '• The file may not be a valid PDF document',
        '• Try providing the correct password if available',
        '',
        `• Error: ${error.message}`,
        `• File size: ${fs.readFileSync(inputFile).length} bytes`,
      ...
      const errorBytes = await errorPdf.save();
      fs.writeFileSync(outputFile, errorBytes);
      return outputFile;
    } catch (fallbackError) {
      console.error('Failed to create error PDF:', fallbackError);
      throw new Error(`Could not unlock PDF: ${error.message}`);
    }
  }
}
